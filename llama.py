...
from llama_chat import format_to_llama_chat_style
...
def get_model_type(model_name):
    if "gptq" in model_name.lower():
        ...
        return ModelType.ggml
    else:
        return ModelType.full_precision
...
def init_auto_model_and_tokenizer(model_name, model_type=None, file_name=None):
    if model_type is None:
        model_type = get_model_type(model_name)

    if ModelType.ggml == model_type:
...
def main(model_name=None, file_name=None):
    ...
    if model_type == ModelType.ggml:
        assert file_name is not None, """When model_name is provided for a GGML quantized model, 
        file_name argument must also be provided."""
